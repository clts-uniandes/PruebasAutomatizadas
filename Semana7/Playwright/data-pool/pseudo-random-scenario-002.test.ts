...
test.describe(`${testCode} - Page Feature`, () => {
    ...
    test.beforeEach( async() => {
        ...
        //Generar data
        pageData.set(PageFields.TITLE, randomElement.useFaker(FakerCategories.WORDS, 1));
        pageData.set(PageFields.CONTENT, randomElement.useFaker(FakerCategories.CHARS, 3000));
        pageData.set(PageFields.PAGE_URL, randomElement.useFaker(FakerCategories.CHARS, 15));
        pageData.set(PageFields.EXCERPT, randomElement.useFaker(FakerCategories.NUMBERS, 100));
    });
